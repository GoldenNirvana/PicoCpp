...
// port  6  1- заморозить,  0-разморозить
{
    ...
    if (flg == 0)
    {
        switch (port)
        {
            case 5:
            {
                io_ports[port].enable();
                afc = " scanner retract " + std::to_string(port);
                break;
            }
            case 6:
            {
                io_ports[port].enable();
                afc = " PID unfreeze " + std::to_string(port);
                break;
            }
        }
    }
    else
    {
        switch (port)
        {
            case 5:
            {
                io_ports[port].disable();
                afc = " scanner protract " + std::to_string(port);
                break;
            }
            case 6:
            {
                io_ports[port].disable();
                afc = " PID  freeze " + std::to_string(port);
                break;
            }
        }
    }
   ...
   */
        ...
        if (CONFIG_UPDATE)
        {
            ...
            if (flgDebug)
            {
                ...
                sendStrData("code" + std::to_string(DEBUG) + " scan parameters update", debugdata, 100,
                            true); //240314 100
            }
            vupdateparams.clear();
    ...
    for (size_t i = 0; i < nslowline; ++i)
    {
        for (uint32_t j = 0; j < nfastline; ++j)
        {
            ...
            else
            {
                ...
                if (conf_.method != oneline)
                {
                    vector_data.emplace_back(int16_t(10000.0 * (sin(w * j) + sin(w * i))));
                }  // get Z from adc
                if (conf_.size == 2)  //дополнительный сигнал
                {
                    vector_data.emplace_back(int16_t(10000.0 * (sin(M_PI * j * 0.1) + sin(M_PI * i * 0.1))));
    ...
    for (uint32_t i = 0; i < nslowline; ++i)
    {
        ...
        for (uint32_t j = 0; j < nfastline; ++j)
        {
            ...
            if (!flgVirtual)
            {
                ...
                { protract(); }// protract(0, ZJump);// вытянуться на ZJump
            }
...
void Scanner::LID_move_toZ0(int lid_name, int f, int p, int n, int dir)  //отвестись в безопасную начальную точку по Z
{
    if (!flgVirtual)
    {
        ...
        sleep_ms(50);
        if (!flgVirtual)
        { linearDriver.activate(lid_name, f, p, std::abs(n), dir); }
        ...
        while (!STOP) //(LID_MOVE_UNTIL_STOP)
        {
            ...
            if (CONFIG_UPDATE)
            {
                ...
                if (flgDebug)
                {
                    ...
                    sendStrData("code" + std::to_string(DEBUG) + " parameters posistionXYZ update", debugdata, 100,
                                true);
                }
                vupdateparams.clear();
            }
        ...
        for (int16_t i = 0; i < NPoints; i++)     //сближение
        {
            ...
            else
            {
                switch (flgModa)
                {
                    case SFM:
                    {
                        SignalValue = ZMaxValue - abs(deltaZ);/*ZMaxValue-dacZ;*/ break;
                    }
                    case STM:
                    case SICMDC:
                    {
        ...
        for (int16_t i = NPoints; i >= 1; i--)  // отвод
        {
            ...
            else
            {
                switch (flgModa)
                {
                    case SFM:
                    {
                        SignalValue = ZMaxValue - abs(deltaZ) + 100;
                        break;
                    }
                    case STM:
                    case SICMDC:
...
void Scanner::testpiezomover(std::vector<int32_t> &vector)
{
    ...
    {
        if (flgstop == 1)
        { break; }                 // если "стоп" был прочитан ранее
        step = -NSTEPS;
