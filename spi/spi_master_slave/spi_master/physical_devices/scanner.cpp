...
void Scanner::start_scan()
{
  ...
  //*//******************************************************************************     
      if (!flgVirtual)
      {
        ...
        if (conf_.sz == 2) //added signal
        {
          other_info.emplace_back((int16_t) spiBuf[conf_.method]);
        }
      } else
      {
    ...
    for (uint32_t j = 0; j <stepslowline; ++j) // go next line
    {
      if (!flgVirtual)
      {
...
void Scanner::approacphm(const int16_t *const data) //uint16_t
{
  ...
  afc.clear();
  afc =
      "code75," + std::to_string(APPROACH) + ',' + std::to_string(buf_status[1]) + ',' + std::to_string(buf_status[2]) +
      "\n";
  std::cout << afc;
  afc.clear();
  sleep_ms(100);
